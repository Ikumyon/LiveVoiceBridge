                    ...
                    while let Ok(adapter) = factory.EnumAdapters1(index) {
                        if let Ok(desc) = adapter.GetDesc1() {
                            ...
                            if (desc.Flags & 2) == 0 {
                                ...
                                let vram_dedicated = desc.DedicatedVideoMemory as f32 / (1024.0 * 1024.0 * 1024.0);
                                let vram_shared = desc.SharedSystemMemory as f32 / (1024.0 * 1024.0 * 1024.0);
                                let total_vram = if vram_dedicated > 0.1 { vram_dedicated } else { vram_shared * 0.5 };
...
impl MetricsProvider for WindowsMetricsProvider {
    fn collect(&mut self) -> MetricsSnapshot {
        let now = Instant::now();
        let dt = now.duration_since(self.last_collect_time).as_secs_f32().max(0.001);
