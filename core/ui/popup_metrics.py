...
class PopupMetricCard(QWidget):
    ...
    def __init__(self, descriptor: MetricDescriptor, mode: str, parent=None) -> None:
        ...
        self.value_label.setStyleSheet(
            f"color: {descriptor.colors[0].name()}; font-weight: bold;"
        )
    ...
    def paintEvent(self, event) -> None:
        ...
        for series_index, history in enumerate(self.histories):
            ...
            for index, current in enumerate(history):
                ...
                points.append(
                    QPointF(left + index * step_x, bottom - normalized * graph_height)
                )
...
class PopupMetricsPanel(QScrollArea):
    ...
    def __init__(self, parent=None) -> None:
        ...
        self.descriptors = {
            descriptor.metric_id: descriptor for descriptor in FIXED_METRIC_DESCRIPTORS
        }
    ...
    def set_display_modes(self, display_modes: dict[str, str]) -> None:
        self.display_modes = {
            str(metric_id): mode
            for metric_id, mode in display_modes.items()
            if mode in POPUP_METRIC_MODES
        }
        self._sync_cards()
    ...
    def _reflow(self, columns: int) -> None:
        ...
        ordered_cards = sorted(
            self.cards.values(), key=lambda card: card.descriptor.order
        )
