...
- `main.py` - アプリ本体
- `live_voice_bridge.ui` - Qt Designerで編集できるUI
- `stream_list.proto` - YouTube公式streamList用のgRPC定義
...
1. VOICEVOXを起動する。
2. `VOICEVOX確認` を押して speaker ID を確認する。
...
- 初回起動時に `stream_list.proto` から `stream_list_pb2.py` と `stream_list_pb2_grpc.py` を自動生成します。
