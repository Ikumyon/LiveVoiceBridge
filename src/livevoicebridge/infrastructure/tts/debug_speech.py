...
from livevoicebridge.application.comment_processing import DictionaryMatcher, replace_emojis
    ...
    for seg in segments:
        ...
        try:
            content = tts_engine.synthesize_wav(
                text=text, speed=target_speed, pitch=seg.get("pitch"), volume=target_volume, speaker_id=target_speaker
            )
            if content:
                ...
                wav_path = apply_audio_effects(
                    wav_path, echo_level=seg.get("echo"), yamabiko_level=seg.get("yamabiko"), panning=seg.get("panning")
                )
