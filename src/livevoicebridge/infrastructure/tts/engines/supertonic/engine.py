...
from livevoicebridge.infrastructure.tts.engines.supertonic import (
    backend_cpu,
    backend_directml,
    backend_openvino,
    backend_openvino_npu,
)
from livevoicebridge.infrastructure.tts.engines.supertonic.common import (
    VOICE_NAMES,
...
class SupertonicEngine(BaseTTSEngine):
    ...
    def available_devices() -> list[tuple[str, str]]:
        return [(device_id, backend.DISPLAY_NAME) for device_id, backend in BACKENDS.items() if backend.is_available()]
    ...
    def synthesize_wav(
        ...
        try:
            ...
            with self._lock:
                ...
                print(f"[Supertonic] TTS入力: {normalized_text} (voice={selected_voice}, lang=ja, steps={total_steps})")
    ...
    def get_speakers(self) -> list[dict] | None:
        return [
            {
                "name": "Supertonic 3",
                "styles": [{"name": name, "id": index} for index, name in enumerate(VOICE_NAMES)],
            }
        ]
