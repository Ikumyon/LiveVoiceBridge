...
class DynamicNpuSession:
    ...
    def run(self, output_names, input_feed):
        shape_key = tuple((name, tuple(value.shape)) for name, value in sorted(input_feed.items()))
        if shape_key not in self.compiled_models:
            model = self.core.read_model(str(self.model_path))
            model.reshape({name: list(value.shape) for name, value in input_feed.items()})
...
def create_tts(model_dir: Path):
    ...
    model_paths = [
        model_dir / relative_path for relative_path in supertonic_loader.get_all_onnx_module_relative_paths()
    ]
    sessions = (
