...
from livevoicebridge_native import (
    hiragana_to_katakana as hiragana_to_katakana,
)
from livevoicebridge_native import (
    parse_comment,
    split_sentences,
...
class DictionaryMatcher:
    ...
    def __init__(self, word_list: list[dict] | None = None):
        entries = [
            (str(item.get("word", "")), str(item.get("reading", ""))) for item in (word_list or []) if item.get("word")
        ]
...
def build_read_text(read_blocks: object, author: str, message: str) -> str:
    parts = []
