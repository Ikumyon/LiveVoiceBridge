...
from livevoicebridge.paths import TASK_MANAGER_UI_FILE
from livevoicebridge.presentation.performance_graph import PerformanceGraphWidget
...
class TaskManagerWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        ui_file = QFile(str(TASK_MANAGER_UI_FILE))
        if not ui_file.open(QFile.OpenModeFlag.ReadOnly):
            raise RuntimeError(f"UIファイルを開けません: {TASK_MANAGER_UI_FILE}")
        self._content = QUiLoader().load(ui_file, self)
        ui_file.close()
        if self._content is None:
            raise RuntimeError("タスクマネージャーUIの読み込みに失敗しました。")
        wrapper_layout = QVBoxLayout(self)
        wrapper_layout.setContentsMargins(0, 0, 0, 0)
        wrapper_layout.addWidget(self._content)

        self.graph_group: QGroupBox = self._content.findChild(QGroupBox, "graphGroup")
        self.grid_layout: QGridLayout = self._content.findChild(QGridLayout, "gridLayout")
        self.comment_queue_label: QLabel = self._content.findChild(QLabel, "commentQueueLabel")
        self.tts_queue_label: QLabel = self._content.findChild(QLabel, "ttsQueueLabel")
        self.speaking_label: QLabel = self._content.findChild(QLabel, "speakingLabel")
        self.yt_conn_label: QLabel = self._content.findChild(QLabel, "youtubeConnectionLabel")
        self.tts_conn_label: QLabel = self._content.findChild(QLabel, "ttsConnectionLabel")
        self.net_sockets_label: QLabel = self._content.findChild(QLabel, "socketLabel")
        ...
        self.concurrent_connections_graph = PerformanceGraphWidget("YouTube 同時接続数", QColor(255, 82, 82), "件", 4.0)
    ...
    def update_metrics(self, data: dict):
        ...
        for idx, gpu_info in enumerate(gpus):
            ...
            if gpu_id not in self.gpu_core_graphs:
                ...
                gpu_graph = PerformanceGraphWidget(
                    clean_name, [QColor(0, 230, 118), QColor(255, 64, 129)], "%", 100.0, self.graph_group
                )
        ...
        if data.get("has_npu", False):
            if self.npu_graph is None:
                self.npu_graph = PerformanceGraphWidget(
                    "NPU 使用率", QColor(179, 136, 255), "%", 100.0, self.graph_group
                )
                # グリッドの末尾に追加
    ...
    def update_connection_info(
        self, is_yt_connected: bool, active_connections_count: int, tts_engine_name: str, socket_count: int
    ):
        """同時接続数および通信状態の更新"""
