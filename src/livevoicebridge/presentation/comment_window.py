...
from PySide6.QtCore import QEvent, QFile, QPoint, Qt, QTimer
from PySide6.QtGui import QColor, QPainter
from PySide6.QtUiTools import QUiLoader
from PySide6.QtWidgets import QPushButton, QSplitter, QVBoxLayout, QWidget

from livevoicebridge.paths import COMMENT_WINDOW_UI_FILE
from livevoicebridge.presentation.popup_metrics import POPUP_METRIC_PLACEMENTS, PopupMetricsPanel
...
class CommentWindow(QWidget):
    ...
    def __init__(self, main_app: LiveVoiceBridgeApp) -> None:
        ...
        # 各種不透明度の設定初期値
        presentation = self.main_app.config.presentation
        self._opacity = presentation.comment_opacity
        self._header_opacity = presentation.header_opacity
        self._border_opacity = presentation.border_opacity

        # ドラッグ移動用の位置保持
        self._drag_pos = QPoint()

        ui_file = QFile(str(COMMENT_WINDOW_UI_FILE))
        if not ui_file.open(QFile.OpenModeFlag.ReadOnly):
            raise RuntimeError(f"UIファイルを開けません: {COMMENT_WINDOW_UI_FILE}")
        self._content = QUiLoader().load(ui_file, self)
        ui_file.close()
        if self._content is None:
            raise RuntimeError("コメントポップアップUIの読み込みに失敗しました。")

        wrapper_layout = QVBoxLayout(self)
        wrapper_layout.setContentsMargins(0, 0, 0, 0)
        wrapper_layout.addWidget(self._content)
        self.header_bar: QWidget = self._content.findChild(QWidget, "headerBar")
        self.close_button: QPushButton = self._content.findChild(QPushButton, "closeButton")
        self.content_splitter: QSplitter = self._content.findChild(QSplitter, "contentSplitter")
        ...
        popup = presentation.popup_metrics
        self._metrics_settings: dict[str, object] = {
            "placement": popup.placement,
            "vertical_ratio": popup.vertical_ratio,
            "horizontal_ratio": popup.horizontal_ratio,
            "display_modes": dict(popup.display_modes),
        }
    ...
    def _configure_splitter(self) -> None:
        ...
        orientation = Qt.Orientation.Vertical if placement in {"top", "bottom"} else Qt.Orientation.Horizontal
    ...
    def _on_splitter_moved(self, position: int, index: int) -> None:
        ...
        ratio_key = (
            "vertical_ratio" if self.content_splitter.orientation() == Qt.Orientation.Vertical else "horizontal_ratio"
        )
        ratio = self._safe_ratio(ratio)
        self._metrics_settings[ratio_key] = ratio
        popup = self.main_app.config.presentation.popup_metrics
        popup = replace(popup, **{ratio_key: ratio})
        self.main_app.config = replace(
            self.main_app.config,
            presentation=replace(self.main_app.config.presentation, popup_metrics=popup),
        )
        ...
        if button == Qt.MouseButton.LeftButton and self._resize_edges:
            ...
            self.main_app.config = replace(
                ...
                presentation=replace(
                    self.main_app.config.presentation,
    ...
    def paintEvent(self, event) -> None:  # noqa: N802
        ...
        bg_hex = self.main_app.config.presentation.background_color
        bg_color = QColor(bg_hex)
        bg_color.setAlpha(int(self._opacity * 255))
        painter.fillRect(self.rect(), bg_color)

        # 縁（境界線）の描画
        border_hex = self.main_app.config.presentation.border_color
    ...
    def wheelEvent(self, event) -> None:  # noqa: N802
        ...
        self.main_app.config = replace(
            self.main_app.config,
            presentation=replace(self.main_app.config.presentation, comment_opacity=self.opacity),
        )
        self.main_app.save_config()
        self.update()
