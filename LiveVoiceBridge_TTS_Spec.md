...
- `supertonic_lightweight` TTSエンジンの追加
...
この構成を維持し、新しいTTSエンジンとして `supertonic_lightweight.py` を追加する。
...
```text
core/tts/engines/supertonic_lightweight.py
```
...
```json
{
  "tts_engine": "supertonic_lightweight",
  "supertonic_lightweight": {
...
| 項目 | 型 | 内容 |
|---|---|---|
| `url` | string | 既存UI互換用。ローカルTTSなので `local://supertonic-lightweight` を使う |
...
`SupertonicLightweightEngine` では次のようにする。
...
## 9. SupertonicLightweightEngine仕様
...
`SupertonicLightweightEngine` 内にロックを持つ。
...
- `tts_engine = supertonic_lightweight` でアプリが起動する
